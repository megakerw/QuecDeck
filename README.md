...
### AT Command Layer
All modem communication goes through [atcli](https://github.com/megakerw/atcli_rust) (a fork of [atcli_rust](https://github.com/1alessandro1/atcli_rust)), a Rust-based AT command CLI that runs as a setuid binary and emits clean newline-terminated output (modem `\r` framing is stripped at the source). Shell code never invokes atcli directly: every caller goes through `script/at-lib.sh` (enforced by a pre-commit check), which routes commands to a queue daemon (`atcmd_queue_daemon.sh`) that serializes requests through named pipes to avoid race conditions. Queued commands carry an expiry deadline so requests nobody is waiting for anymore are skipped instead of sent to the modem. Responses are cached per endpoint to reduce modem load. Cache TTLs are tuned to how often data actually changes: 3 seconds for signal stats and connection info, 5 seconds for network and settings data, and 1 hour for static device info like firmware version and build time. Where possible, multiple AT commands are batched into a single request to cut down on round trips.
...
- **On-device scripts** (`tools/device-test-*.sh`): copied to the device manually for what host tests cannot verify: SELinux domain interactions, the setuid path, and real modem timing and payload sizes. Run before tagging a release.

The pre-commit hook is enabled with `git config core.hooksPath .githooks`.

**Shell performance rule** (measured on the device's single Cortex-A7): bash builtins are used only for small or fixed-size data; bulk transformation of unbounded AT responses uses `tr`/`awk`, never bash pattern replacement, whose cost scales with size times match count (48 seconds on a 44 KB SMS list, versus 20 ms for `tr`). CI guards the queue path with a large-response test.
